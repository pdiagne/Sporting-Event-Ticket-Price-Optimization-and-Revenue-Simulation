...
# Define sections based on Dodger Stadium's actual layout
    # The total capacity of these sections is 56,000
sections = {
...
# Calculate initial prices based on normalized utility and base price
for section in sections:
    normalized_utility = sections[section]['normalized_utility']
    base_price = sections[section]['base_price']

    # Adjust price based on utility (higher utility = higher price)
    price_adjustment = normalized_utility * 0.5  # 50% maximum adjustment
    initial_price = base_price * (1 + price_adjustment)

    sections[section]['initial_price'] = round(initial_price, 2)

# Create a DataFrame for easy viewing
df = pd.DataFrame.from_dict(sections, orient='index')
df = df.sort_values('initial_price', ascending=False)

print(df[['base_price', 'initial_price', 'capacity', 'normalized_utility']])

# Calculate potential revenue
total_potential_revenue = sum(section['initial_price'] * section['capacity'] for section in sections.values())
print(f"\nTotal potential revenue: ${total_potential_revenue:,.2f}")


# Create the bar chart for optimal price per section
plot_dir = script_dir
section_names = list(sections.keys())
initial_prices = [sections[section]['initial_price'] for section in sections]
plt.figure(figsize=(10, 6))
plt.bar(section_names, initial_prices, color='skyblue')
plt.title('Optimal Price per Section')
plt.xlabel('Section')
plt.ylabel('Price ($)')
plt.xticks(rotation=45, ha='right')

output_file = os.path.join(plot_dir, 'optimal_ticket_prices.png')
plt.tight_layout()
...
sections = {
    'Field': {'initial_price': df['initial_price'].iloc[0], 'capacity': 15000, 'utility': part_worth[1][0]},  # example initial_price
    'Loge': {'initial_price': df['initial_price'].iloc[1], 'capacity': 15000, 'utility': part_worth[1][1]},  # example initial_price
    'Reserved': {'initial_price': df['initial_price'].iloc[2], 'capacity': 15000, 'utility': part_worth[1][2]},  # example initial_price
    'Top Deck': {'initial_price': df['initial_price'].iloc[3], 'capacity': 11000, 'utility': part_worth[1][3]},  # example initial_price
}
